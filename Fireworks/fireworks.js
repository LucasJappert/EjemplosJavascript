...
const frameRate = 120.0;							// Frame rate per second use in loop
...
Particles = function() {
    ...
    this.update = function() {
        ...
        if (this.s > 900/frameDelay) { if (this.opacity - 0.05 < 0) { this.opacity = 0; } else { this.opacity -= 0.05; } }
        ...
        if (this.type == 1) {
            ...
            if (this.s < 800/frameDelay) { this.scale += 0.1; } else { this.scale -= 0.2; }
            this.speed *= 0.96;
            this.x -= this.vx * this.speed * this.ms + this.wind;
            this.y -= this.vy * this.speed * this.ms - this.gravity;
        } else if (this.type == 3) {
...
update = function() {
    ...
    if (timer > LimiterTicker) {
        ...
        LimiterTicker = timer + (TimedFirework / frameDelay);
    }

    var i = fireworks.length;
    while(i--) {
        // Progress starting Fireworks
        if (fireworks[i].del) { fireworks.splice(i, 1); } else {
            fireworks[i].update();
            fireworks[i].draw();
        }
    }
...
var main = setInterval(function() { update(); }, frameDelay);